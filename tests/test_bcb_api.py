...
class TestCreateApiUrl(unittest.TestCase):
    """ Class to test the _create_pi_url() method from FinancialIndicatorsApi class."""

    def setUp(self) -> None:
        """ Instantiate FinancialIndicatorsApi for each test."""
        self.bcb_api = FinancialIndicatorsApi()
...
class TestGetJsonResults(unittest.TestCase):
    """ Class to test the _get_json_results() method from FinancialIndicatorsApi."""

    def setUp(self) -> None:
        """ Instantiate FinancialIndicatorsApi for each test."""
        self.bcb_api = FinancialIndicatorsApi()

    # Testing for daily indicators.

    def test_daily_indicator_selic_format(self):
    ...
    def test_daily_indicator_cdi_format(self):
    ...
    def test_daily_indicator_tr_format(self):
    ...
    def test_daily_indicator_same_valid_dates(self):
    ...
    def test_daily_indicator_same_invalid_dates(self):
        """ Since the results from the dates are inclusive, when one date is
        given as both start and end_date, one record is found.

        If the date given doesn't have a value (holiday or weekend, depending
        on the indicator), the api searches for the last valid date and returns
        that value.
        """
    ...
    def test_daily_indicator_start_date_none_cdi(self):
    ...
    def test_daily_indicator_end_date_none_selic(self):
        ...
        expected = {
            'data': '04/06/1986', 'valor': '0.065041'  # first available record for this indicator
        }
        actual = self.bcb_api._get_json_results(url)[0]  # first record

        self.assertEqual(expected, actual)

    def test_daily_indicator_lengthy_result_selic(self):
    ...
    # Testing for monthly indicator.

    def test_monthly_indicator_ipca_format(self):
    ...
    def test_monthly_indicator_ipca15_format(self):
    ...
    def test_monthly_indicator_same_valid_dates(self):
    ...
    def test_monthly_indicator_start_date_none_ipca(self):
        """ When start_date is an invalid value (such as an alphabetic character
        or None), the api will query all results, not respecting the end_date.
    ...
    def test_monthly_indicator_end_date_none_ipca(self):
        """ When start_date is an invalid value (such as an alphabetic character
        or None), the api will query all results, not respecting the end_date.
...
class TestFixApiResultsTwoFields(unittest.TestCase):
    """ Class to test the _fix_api_results() method from FinancialIndicatorsApi,
    where each record has only two fields of values (date and value)."""

    def setUp(self) -> None:
        """ Instantiate FinancialIndicatorsApi, and define the namedtuple type that
         results from the method, for each test.
         """
        self.bcb_api = FinancialIndicatorsApi()
        self.IndicatorRecord = namedtuple('IndicatorRecord', ('date', 'value'))
    ...
    def test_one_record_result(self):
        ...
        expected = [
            self.IndicatorRecord(date=datetime.date(1992, 3, 5), value=1.250667),
        ]
        actual = self.bcb_api._fix_api_results(argument)
    ...
    def test_result_five_records(self):
        ...
        expected = [
            self.IndicatorRecord(date=datetime.date(2008, 12, 26), value=0.050299),
            self.IndicatorRecord(date=datetime.date(2008, 12, 29), value=0.050578),
            self.IndicatorRecord(date=datetime.date(2008, 12, 30), value=0.050648),
            self.IndicatorRecord(date=datetime.date(2008, 12, 31), value=0.050683),
            self.IndicatorRecord(date=datetime.date(2009, 1, 2), value=0.050683),
        ]
        actual = self.bcb_api._fix_api_results(argument)
    ...
    def test_result_five_records_inverted(self):
        ...
        expected = [
            self.IndicatorRecord(date=datetime.date(2012, 7, 1), value=0.43),
            self.IndicatorRecord(date=datetime.date(2012, 8, 1), value=0.41),
            self.IndicatorRecord(date=datetime.date(2012, 9, 1), value=0.57),
            self.IndicatorRecord(date=datetime.date(2012, 10, 1), value=0.59),
            self.IndicatorRecord(date=datetime.date(2012, 11, 1), value=0.60),
        ]
        actual = self.bcb_api._fix_api_results(argument)
...
class TestFixApiResultsThreeFields(unittest.TestCase):
    ...
    def setUp(self) -> None:
        """ Instantiate FinancialIndicatorsApi, and define the namedtuple type that
         ...
         """
        self.bcb_api = FinancialIndicatorsApi()
        self.IndicatorRecord = namedtuple('IndicatorRecord',
                                        ('date', 'end_date', 'value'))
    ...
    def test_result_one_record(self):
        ...
        expected = [
            self.IndicatorRecord(date=datetime.date(1992, 5, 3), end_date=datetime.date(1992, 6, 3), value=22.0529),
        ]
        actual = self.bcb_api._fix_api_results(argument)
    ...
    def test_result_five_records(self):
        ...
        expected = [
            self.IndicatorRecord(date=datetime.date(2008, 12, 26), end_date=datetime.date(2009, 1, 26),  value=0.1478),
            self.IndicatorRecord(date=datetime.date(2008, 12, 27), end_date=datetime.date(2009, 1, 27),  value=0.1560),
            self.IndicatorRecord(date=datetime.date(2008, 12, 28), end_date=datetime.date(2009, 1, 28),  value=0.1844),
            self.IndicatorRecord(date=datetime.date(2008, 12, 29), end_date=datetime.date(2009, 1, 29),  value=0.1930),
            self.IndicatorRecord(date=datetime.date(2008, 12, 30), end_date=datetime.date(2009, 1, 30),  value=0.2235),
        ]
        actual = self.bcb_api._fix_api_results(argument)
    ...
    def test_result_three_records_inverted(self):
        ...
        expected = [
            self.IndicatorRecord(date=datetime.date(2009, 4, 21), end_date=datetime.date(2009, 5, 21), value=0.0533),
            self.IndicatorRecord(date=datetime.date(2009, 4, 22), end_date=datetime.date(2009, 5, 22), value=0.0474),
            self.IndicatorRecord(date=datetime.date(2009, 4, 23), end_date=datetime.date(2009, 5, 23), value=0.0783),
        ]
        actual = self.bcb_api._fix_api_results(argument)
...
class TestRmRecordsOutsideRange(unittest.TestCase):
    ...
    def test_empty_dates_one_record(self):
        ...
        """
        records = [self.IndicatorRecord(date=datetime.date(1986, 6, 4), value=0.065041),]
        expected = records[:]
    ...
    def test_empty_dates_few_records(self):
        ...
        """
        records = [
            self.IndicatorRecord(date=datetime.date(2018, 12, 14), value=0.024620),
            self.IndicatorRecord(date=datetime.date(2018, 12, 17), value=0.024620),
            self.IndicatorRecord(date=datetime.date(2018, 12, 18), value=0.024620),
            self.IndicatorRecord(date=datetime.date(2018, 12, 19), value=0.024620),
        ]
        expected = records[:]
        actual = self.bcb_api._rm_records_outside_range(None, None, records)
    ...
    def test_first_record_equal_to_start_date(self):
        ...
        records = [  # result from api
            self.IndicatorRecord(date=datetime.date(2014, 4, 10), value=0.040705),
            self.IndicatorRecord(date=datetime.date(2014, 4, 11), value=0.040705),
            self.IndicatorRecord(date=datetime.date(2014, 4, 14), value=0.040705),
            self.IndicatorRecord(date=datetime.date(2014, 4, 15), value=0.040705),
            self.IndicatorRecord(date=datetime.date(2014, 4, 16), value=0.040705),
            self.IndicatorRecord(date=datetime.date(2014, 4, 17), value=0.040705),
            self.IndicatorRecord(date=datetime.date(2014, 4, 22), value=0.040705),
            self.IndicatorRecord(date=datetime.date(2014, 4, 23), value=0.040705),
        ]
        expected = records[:]
    ...
    def test_first_record_lower_than_start_date(self):
        ...
        """
        records = [  # result from api
            self.IndicatorRecord(date=datetime.date(2011, 12, 30), value=0.040956),
            self.IndicatorRecord(date=datetime.date(2012, 1, 2), value=0.041028),
            self.IndicatorRecord(date=datetime.date(2012, 1, 3), value=0.040992),
        ]
        expected = [
            self.IndicatorRecord(date=datetime.date(2012, 1, 2), value=0.041028),
            self.IndicatorRecord(date=datetime.date(2012, 1, 3), value=0.040992),
    ...
    def test_last_record_equal_to_end_date(self):
        ...
        records = [  # result from the api
            self.IndicatorRecord(date=datetime.date(1986, 6, 4), value=0.065041),
            self.IndicatorRecord(date=datetime.date(1986, 6, 5), value=0.067397),
            self.IndicatorRecord(date=datetime.date(1986, 6, 6), value=0.066740),
        ]
        expected = records[:]
    ...
    def test_last_record_higher_than_end_date(self):
        ...
        records = [  # result from the api
            ...
            self.IndicatorRecord(date=datetime.date(1986, 6, 5), value=0.067397),
            self.IndicatorRecord(date=datetime.date(1986, 6, 6), value=0.066740),
            self.IndicatorRecord(date=datetime.date(1986, 6, 9), value=0.068247),
            self.IndicatorRecord(date=datetime.date(1986, 6, 10), value=0.067041),
        ]
        expected = records[:-2]
        actual = self.bcb_api._rm_records_outside_range(None,
                                                        datetime.date(1986, 6, 6),
    ...
    def test_valid_dates_with_no_removals(self):
        ...
        """
        records = [
            self.IndicatorRecord(date=datetime.date(2007, 7, 26), value=0.058058),
            self.IndicatorRecord(date=datetime.date(2007, 7, 27), value=0.058058),
            self.IndicatorRecord(date=datetime.date(2007, 7, 28), value=0.058092),
            self.IndicatorRecord(date=datetime.date(2007, 7, 29), value=0.058160),
            self.IndicatorRecord(date=datetime.date(2007, 7, 30), value=0.058298),
            self.IndicatorRecord(date=datetime.date(2007, 8, 2), value=0.058298),
        ]
        expected = records[:]
...
class TestGetLatestDateTwoFields(unittest.TestCase):
    """ Class to test get_latest_date() method from the FinancialIndicatorsApi
    class.
    """
    def setUp(self) -> None:
        """ Instantiate FinancialIndicatorsApi for each test."""
        ...
        self.IndicatorRecord = namedtuple('IndicatorRecord',
                                        ('date', 'value'))
        self.bcb_api._indicators_records = {
            11: [
                self.IndicatorRecord(date=datetime.date(2005, 9, 27), value=0.070718),
                self.IndicatorRecord(date=datetime.date(2005, 9, 28), value=0.070784),
                self.IndicatorRecord(date=datetime.date(2005, 9, 29), value=0.070784),
                self.IndicatorRecord(date=datetime.date(2005, 9, 30), value=0.070818),
            ],
            12: [
                # Indicator without a record
            ],
            433: [
                self.IndicatorRecord(date=datetime.date(1987, 4, 1), value=19.10)
            ],
        }

    def test_non_existing_indicator(self):
        """ When a non-existing indicator is searched, it should return None."""
        expected = None
        actual = self.bcb_api.get_latest_date(1)

        self.assertEqual(expected, actual)

    def test_indicator_without_records(self):
        """ If an indicator do exist, but it doesn't have a value, it should
        return None."""
        expected = None
    ...
    def test_indicator_with_single_record(self):
        """ get_latest_date() should return the date of the latest record,
        even if that indicator has only one record.
        """
    ...
    def test_indicator_with_multiple_records(self):
        """ get_latest_date() should return the date of the latest record."""
...
class TestGetLatestDateThreeFields(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.bcb_api._indicators_records = {
            226: [
                self.IndicatorRecord(date=datetime.date(1991, 2, 1), end_date=datetime.date(1991, 3, 1), value=7.0000),
                self.IndicatorRecord(date=datetime.date(1991, 2, 2), end_date=datetime.date(1991, 3, 2), value=7.4604),
                self.IndicatorRecord(date=datetime.date(1991, 2, 3), end_date=datetime.date(1991, 3, 3), value=7.4604),
                self.IndicatorRecord(date=datetime.date(1991, 2, 4), end_date=datetime.date(1991, 3, 4), value=7.4604),
                self.IndicatorRecord(date=datetime.date(1991, 2, 5), end_date=datetime.date(1991, 3, 5), value=7.6135),
            ],
            253: [
                self.IndicatorRecord(date=datetime.date(1998, 3, 30), end_date=datetime.datetime(1998, 4, 30), value=1.7585)
            ],
            25: [
...
class TestSetIndicatorRecords(unittest.TestCase):
    """ Class to test the set_indicators_records() method from the
    FinancialIndicatorsApi class.
    """
    ...
    def setUp(self) -> None:
        ...
        self.bcb_api = FinancialIndicatorsApi()
        self.bcb_api.set_indicators_records(arguments)

    def test_start_date_end_date_as_none(self):
        """ Test both the first and last date from the ipca-15 indicator, when
        both dates are None, and all available results are retrieved.
        """
        today = datetime.date.today()
        expected = (datetime.date(2000, 5, 1), True)
        actual = (self.bcb_api._indicators_records[7478][0].date,
                  self.bcb_api._indicators_records[7478][-1].date <= today,
                  )

        self.assertEqual(expected, actual)

    def test_start_date_as_none(self):
        """ Test the first date from ipca indicator as None."""
        today = datetime.date.today()
        expected = (datetime.date(1980, 1, 1), True)
        actual = (self.bcb_api._indicators_records[433][0].date,
                  self.bcb_api._indicators_records[433][-1].date <= today,
                  )
    ...
    def test_end_date_as_none(self):
        ...
        actual = (self.bcb_api._indicators_records[11][0].date,
                  self.bcb_api._indicators_records[11][-1].date <= today,
                  )
    ...
    def test_valid_dates_two_field(self):
        ...
        actual = (self.bcb_api._indicators_records[12][0].date,
                  self.bcb_api._indicator_records[12][-1].date,
                  )
    ...
    def test_valid_dates_three_field(self):
        ...
        actual = (self.bcb_api._indicator_records[226][0].date,
                  self.bcb_api._indicators_records[226][-1].date,
                  )
